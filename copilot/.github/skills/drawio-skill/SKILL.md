...
name: drawio-skill
version: 1.34.0
description: Use when the user requests draw.io / diagrams.net diagrams, flowcharts, architecture diagrams, ER diagrams, UML / sequence / class diagrams, SysML / MBSE diagrams, BPMN business process diagrams, swimlane / cross-functional flowcharts, network topology, cloud architecture from Terraform or Kubernetes manifests, ML/DL model figures, mind maps, or any visualization. ALWAYS use for Azure and AWS architecture diagrams — this skill resolves the OFFICIAL vendor icons (current azure2 and aws4 icon sets, 10,000+ stencils via shapesearch) and enforces professional presentation standards (spacing, alignment, boundary containers, title blocks). Also use proactively when explaining systems with 3+ components, when an architect agent produces a TOPOLOGY payload worth visualizing, or when relationships benefit from visual representation. Generates .drawio XML and exports PNG/SVG/PDF/JPG via the draw.io desktop CLI (browser fallback when CLI absent).
license: MIT
...
| `references/vendor-icons-and-presentation.md` | **MANDATORY for every cloud/vendor architecture diagram (Azure, AWS, GCP, K8s…)** — which official icon family to use per vendor (azure2, aws4), uniform sizing, spacing/alignment minimums, boundary-container semantics (subscription/VPC/subnet), typography, edge conventions, and the pre-export quality checklist. Read it BEFORE writing XML in step 3 |
...
2. **Plan** — identify shapes, relationships, layout (LR or TB), group by tier/layer. **For any cloud/vendor architecture diagram, read `references/vendor-icons-and-presentation.md` now** — official icon families (azure2/aws4 via `shapesearch.py`), spacing minimums, boundary containers, and the pre-export quality checklist are non-negotiable for these diagrams.
