...
description: "Senior Terraform Engineer (Codename: Atlas). Use PROACTIVELY whenever Terraform/IaC must be written, reviewed, refactored, imported, tested, or validated. Consumes REQ/TOPOLOGY/CONSTRAINTS payloads from the architect agents, writes modules and roots, runs terraform fmt/validate/test plus tflint/tfsec/checkov, and routes plans to plan-mode-reviewer before any apply."
